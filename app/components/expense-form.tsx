...
type Action =
  ...
  | { type: 'set_entry_type'; value: 'expense' | 'income' };
...
export function ExpenseForm({
  ...
  return (
    ...
    >
      <input type="hidden" name="entryType" value={state.entryType} />
      {selectedMonth && (
        ...
        <div className="grid grid-cols-2 gap-1 rounded-xl bg-slate-100 p-1">
          {[
            ...
            <label key={option.value} className="cursor-pointer">
              <input
                type="radio"
                name="entryTypeToggle"
                value={option.value}
                className="peer sr-only"
                checked={state.entryType === option.value}
                onChange={() =>
                  dispatch({
                    type: 'set_entry_type',
                    value: option.value,
              ...
              />
              <div className="rounded-lg py-2 text-center text-xs font-medium transition-colors peer-checked:bg-slate-900 peer-checked:text-white text-slate-600">
                {option.label}
              </div>
            </label>
          ))}
