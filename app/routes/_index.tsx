...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  return data({
    months,
    activeMonth,
    defaultSource: cookieSource ?? 'Suami',
  });
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const raw = {
    ...
    entryType: (formData.get('entryType') as string) || 'expense',
  };
  ...
  const entryTypeLabel =
    parsed.entryType === 'income' ? 'Pemasukan' : 'Pengeluaran';
  ...
  async function handleOfflineSubmit(formData: FormData) {
    const expense = {
      ...
      formData: {
        ...
        method: formData.get('method') as string,
        source: formData.get('source') as string,
        entryType: (formData.get('entryType') as 'expense' | 'income') ?? 'expense',
      },
    };
